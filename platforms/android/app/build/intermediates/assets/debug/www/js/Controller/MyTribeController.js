...
//         跳转
    ...
    // 托管或取消托管 wuyabuluo
    // status: 1取消托管 0非托管
    $scope.trustship = function(selectLandId, status, index){
        // 确认对话框
        ...
        confirmPopup.then(function(res) {
            if(res) {
                console.log('确定');
                GreenhouseService.trusteeship(houseId,selectLandId, status)
                    .success(function(data){
                        if(data.code == 200){
                            var alertPopup = $ionicPopup.alert({
                                title: '提示',
                            ...
                            //$scope.data.selectTransfer = status;
                            //console.log("信息1---------》" + JSON.stringify($scope.tribe.myGreenHouseLand[index]));
                            $scope.tribe.myGreenHouseLand[index].trusteeship = status;
                        }
                        else{
                            var alertPopup = $ionicPopup.alert({
                                title: '提示',
